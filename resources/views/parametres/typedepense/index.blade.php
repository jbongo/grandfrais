...
@section('content')
    <div class="content">
                ...
                <div class="page-title-box">
                    <div class="page-title-right">
                        <ol class="breadcrumb m-0">
                            <li class="breadcrumb-item"><a href="">Type de dépense</a></li>
                        </ol>
                    </div>
                    <h4 class="page-title">Type de dépense</h4>
                </div>
            </div>
        </div>
        <!-- end page title -->
                    ...
                    <div class="card-body p-0">
                        <div class="row g-0">
                            ...
                            <div class="col-sm-2 mr-14 ">
                                <a href="{{ route('depense.index') }}" type="button" class="btn btn-outline-primary"><i
                                        class="uil-arrow-left"></i> Dépenses</a>
                            </div>
                            @if (session('ok'))
                                <div class="col-6">
                                ...
                                <tbody>
                                    @foreach ($typedepenses as $typedepense)
                                        <tr>
                                            ...
                                            <td><a href="#" class="text-body fw-bold">{{ $typedepense->type }}</a> </td>
                                            <td>
                                                @if ($typedepense->archive == false)
                                            ...
                                            <td>
                                                ...
                                                <a data-href="{{ route('typedepense.update', Crypt::encrypt($typedepense->id)) }}"
                                                    data-value="{{ $typedepense->type }}" data-bs-toggle="modal"
                                                    data-bs-target="#edit-modal" class="action-icon edit-type text-success">
                                                    <i class="mdi mdi-square-edit-outline"></i></a>
                                                @if ($typedepense->archive == false)
                                                    <a data-href="{{ route('depense.archive', $typedepense->id) }}"
                                                        style="cursor: pointer;"
                                                        class="action-icon archive-type text-warning"> <i
                                                            class="mdi mdi-archive-arrow-down"></i></a>
                                                @else
                                                    <a data-href="{{ route('depense.unarchive', $typedepense->id) }}"
                                                        style="cursor: pointer;"
                                                        class="action-icon unarchive-type text-success"> <i
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="standard-modalLabel">Ajouter un type de dépense</h4>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>

                    <form action="{{ route('typedepense.store') }}" method="post">
        ...
        {{-- Modification d'un type de dépense --}}
        <div id="edit-modal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="standard-modalLabel"
            aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="standard-modalLabel">Modifier le type de dépense *</h4>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    ...
                    <form action="" method="post" id="form-edit">
                        <div class="modal-body">
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="form-floating mb-3">
                                    <input type="text" name="type" value="{{ old('type') ? old('type') : '' }}"
                                        class="form-control" id="edit-type" required>
                                    <label for="edit-type">Type de dépense</label>
                                    @if ($errors->has('type'))
                                        <br>
                                        <div class="alert alert-warning text-secondary " role="alert">
                                            <button type="button" class="btn-close btn-close-white"
...
@section('script')
    ...
    <script>
        $(function() {
            ...
            $('body').on('click', 'a.archive-type', function(event) {
                let that = $(this)
    ...
    <script>
        ...
        $(function() {
            ...
            $('[data-toggle="tooltip"]').tooltip()
            $('body').on('click', 'a.unarchive-type', function(event) {
                let that = $(this)
                event.preventDefault();
                ...
                swalWithBootstrapButtons.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        swalWithBootstrapButtons.fire(
                            'Annulé',
                            'Type de dépense non désarchivé :)',
                            'error'
                        )
                    }
                });
